...
export function StateRequirementsSection({ data }: { data: StateData }) {
  ...
  const requirements = data.requirementsItems && data.requirementsItems.length > 0
    ? data.requirementsItems
    : defaultRequirements;

  const title = data.requirementsTitle || `Basic Requirements For ${stateName} ESA Letter`;
  const intro = data.requirementsIntro;
  const note = data.requirementsNote || `ESA evaluation in ${stateName} must be determined through an individualized assessment conducted by an ${stateName}-licensed mental health professional (LMHP). An ESA letter may only be issued when clinically appropriate based on that evaluation.`;
    ...
    <section className="w-full bg-[#FAF7F2] py-10 sm:py-14 lg:py-16 px-4 sm:px-6 lg:px-8">
      <div className="max-w-[895px] mx-auto">
        <div className="bg-[rgba(232,185,44,0.1)] rounded-[30px] p-6 sm:p-8 lg:p-10 shadow-sm text-center">
          <h2 className="font-heading text-xl sm:text-2xl lg:text-[24px] xl:text-[28px] font-bold text-[#2E5A66] text-center mb-3 sm:mb-4 tracking-tight">
            {title}
          </h2>

          {intro && (
            <p className="font-sans text-xs sm:text-sm lg:text-[14px] font-semibold text-[#5F6B6F] leading-relaxed max-w-[720px] mx-auto mb-6 text-center">
              {intro}
            </p>
          )}
