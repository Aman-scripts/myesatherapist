...
import { SectionBadge, GoldLipBadge, FocusCard } from "@/components/legal-reviewer/ReviewerUI";
import { AuthorProfile } from "@/data/authorsData";

function ProfileDetails({ author }: { author: AuthorProfile }) {
  return (
    <>
      {/* Badges: Active Now & Experience */}
      ...
      <div className="space-y-4">
        ...
        <ul className="space-y-3 font-lato text-[14px] leading-[22px] text-[#FAF7F2] tracking-[-0.017em]">
          {author.sidebarEducation.map((item) => (
            <li key={item.label} className="flex items-center justify-between gap-4">
              <span>{item.label}</span>
              <span className="font-medium text-[#FAF7F2]/90">{item.year}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      <div className="space-y-3">
        ...
        <ul className="space-y-2 font-lato text-[14px] leading-[22px] text-[#FAF7F2] tracking-[-0.017em]">
          {author.sidebarFocusAreas.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
      </div>
...
export function AuthorProfileContent({ author }: { author: AuthorProfile }) {
  ...
  return (
    <section className="w-full bg-[#FAF7F2] relative pb-16 lg:pb-24">
      <div className="w-full flex flex-col lg:flex-row items-start justify-start">
        {/* LEFT COLUMN (Profile Sidebar) - DESKTOP */}
        <aside
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar p-6 sm:p-8 lg:px-[31px] lg:pt-2 lg:pb-10 space-y-6">
            <ProfileDetails author={author} />
          </div>
        </aside>

        {/* MOBILE & TABLET TRIGGER + PORTAL DRAWER */}
        {mounted &&
          createPortal(
            <>
              <button
              ...
              <div
                ...
                role="dialog"
                aria-modal="true"
                aria-label="Author Profile Drawer"
              >
                <div
        ...
        <div className="flex-1 min-w-0 max-w-[1055px] px-4 sm:px-8 lg:px-[74px] pt-8 lg:pt-[50px] pb-0 space-y-10 lg:space-y-[48px]">
          ...
          <div className="space-y-6 lg:space-y-7">
            ...
            {author.bioParagraphs.map((para, idx) => (
              <p
                key={idx}
                className="font-sans font-semibold text-[16px] sm:text-[18px] leading-[28px] sm:leading-[30px] text-[#5F6B6F]"
              >
                {para}
              </p>
            ))}

            {/* Skill Tags or Focus Cards */}
            {author.focusCards ? (
              <div className="flex flex-wrap items-center gap-4 sm:gap-5 pt-2">
                {author.focusCards.map((card) => (
                  <FocusCard
                    key={card.title}
                    title={card.title}
              ...
              <div className="flex flex-wrap items-center gap-2.5 pt-2">
                {author.skillTags?.map((tag) => (
                  <GoldLipBadge key={tag}>{tag}</GoldLipBadge>
                ))}
              </div>
            )}
          </div>
          ...
          <div className="space-y-8 lg:space-y-10">
            ...
            {author.educationDetails.map((edu) => (
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-2.5 gap-x-8 lg:gap-x-16 font-lato text-[15px] sm:text-[16px] leading-[30px] sm:leading-[32px] text-[#5F6B6F] tracking-[-0.017em]">
              <ul className="space-y-2.5">
                {author.focusAreas
                  .slice(0, Math.ceil(author.focusAreas.length / 2))
                  .map((item) => (
              ...
              <ul className="space-y-2.5">
                {author.focusAreas
                  .slice(Math.ceil(author.focusAreas.length / 2))
                  .map((item) => (
                    <li key={item} className="flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#5F6B6F] shrink-0" />
                      <span>{item}</span>
          ...
          {/* CONTENT SECTIONS (Role / Editorial Standards / Commitment) */}
          {author.contentSections.map((section) => (
            <React.Fragment key={section.heading}>
              <hr className="border-t border-[#5F6B6F]/25" />
              <div className="space-y-5">
                <SectionBadge title={section.heading} />
                {section.intro && (
                ...
                {section.bullets && (
                  <ul className="space-y-2.5 font-lato text-[15px] sm:text-[16px] leading-[26px] text-[#5F6B6F] tracking-[-0.017em]">
                    {section.bullets.map((b) => (
                      <li key={b} className="flex items-start gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#5F6B6F] shrink-0 mt-2.5" />
                        <span>{b}</span>
                      </li>
                    ))}
                  </ul>
          ...
          {/* SECTION: DISCLAIMER */}
          <div
            className="rounded-[30px] p-6 sm:p-8 lg:p-[37px] text-[#FAF7F2] shadow-sm space-y-3.5"
            style={{
              background: "linear-gradient(135deg, #1A3D4F 0%, #1D6E72 100%)",
            }}
          >
            ...
            {author.disclaimerParagraphs.map((para, idx) => (
              <p
                key={idx}
                className="font-lato text-[14px] sm:text-[16px] leading-[24px] sm:leading-[26px] text-[#FAF7F2] tracking-[-0.017em]"
              >
                {para}
              </p>
            ))}
          ...
          {/* SECTION: ARTICLES NOTE */}
          {author.articlesNote && (
            <div className="space-y-4">
              <SectionBadge title={`Articles by ${author.name}`} />
              <p className="font-sans font-semibold text-[16px] sm:text-[18px] leading-[28px] sm:leading-[30px] text-[#5F6B6F]">
                {author.articlesNote}
              </p>
            </div>
